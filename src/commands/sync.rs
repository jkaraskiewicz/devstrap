...
use crate::cli::Cli;
use crate::commands::{run_installation, run_runtime_installation};
use crate::init::{initialize_app, load_system_and_config};
use colored::Colorize;
use devstrap::detect::SystemInfo;
use devstrap::{confirm, Config, Installer, StateFile};
use std::path::Path;
use std::process;
...
fn attempt_uninstall(pkg_id: &str, method_str: &str, system_info: &SystemInfo, state: &mut StateFile) {
    let method = devstrap::InstallMethod::from_string(&method_str.to_lowercase(), system_info);
    if let Some(method) = method {
        if let Err(e) = devstrap::utils::uninstall_package(pkg_id, &method) {
...
fn update_state_for_installed(
    ...
    for pkg_id in to_install {
        let method = system_info
            ...
            .map(devstrap::InstallMethod::SystemDefault)
            .unwrap_or(devstrap::InstallMethod::System);
